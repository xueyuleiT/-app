...
//	private GifView login_gif;
    ...
    private boolean isCheck = true, isFirstRun = false;
	@Override
...
//             
//             try{
//            	 File file = new File(Environment.getExternalStorageDirectory() + "/anchang/"+"login.gif");  
//  				 file.createNewFile();
//                 bis=new BufferedInputStream(getResources().openRawResource(R.drawable.login));
//                 fos=new FileOutputStream(Environment.getExternalStorageDirectory() + "/anchang/"+"login.gif");
...
//		}
//		try {
//			is = new FileInputStream(Environment.getExternalStorageDirectory() + "/anchang/"+"login.gif");
//		} catch (FileNotFoundException e) {
//			e.printStackTrace();
//		}
...
//			settlementPop.showAtLocation(settlementView.findViewById(R.id.text), Gravity.CENTER_HORIZONTAL, location[0], location[1]);
//	}
	private void initUI() {
...
//		login_gif.setShowDimension(mDisplayMetrics.widthPixels, mDisplayMetrics.heightPixels);
//		login_gif.destroyDrawingCache();
		edtName.setText(sp.getString("USER_PHONE", ""));
		...
		login.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if(!leibie.getText().toString().equals("")){
					PhoneReceiver.showLoad(Login.this,login);
	 ...
	 public boolean isFileExist(String fileName){  
	        File file = new File(Environment.getExternalStorageDirectory() + "/anchang/" + fileName);  
	        return file.exists();  
	    } 
    public  File createSDDir(){  
        File dir = new File(Environment.getExternalStorageDirectory() + "/anchang") ;  
        if(!dir.exists())
        	dir.mkdirs();  
        return dir;  
    }
	...
	private boolean login(){
			...
			isFirstRun = sp.getBoolean("isFirstRun", true);
				...
				try {
					TextMessage textMsg = NetworkData.posturl(Constant.LOGIN+"?id="+edtName.getText()
							.toString().trim()+"&password="+edtPwd.getText().toString().trim());
					System.out.println("content = "+textMsg.getContent());
					if(textMsg != null && textMsg.getContent().contains("登陆失败")){
						finish();
						return false;
						...
						if(isFirstRun || !edtName.getText().toString().equals(sp.getString("USER_PHONE", ""))){
							editor.putBoolean("isFirstRun", false);
							editor.putString("USER_PHONE", edtName.getText().toString());
							...
							Constant.SAFEPWD = sp.getString("SAFE_PWD", "");
